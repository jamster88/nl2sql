...
**Diagram:** [`Multi-Agent_NL2SQL_arch5.drawio`](Multi-Agent_NL2SQL_arch5.drawio)
(editable in draw.io) and [`Multi-Agent_NL2SQL_arch5.png`](Multi-Agent_NL2SQL_arch5.png)
(rendered). The draw.io file is generated by
[`make_arch5_drawio.py`](make_arch5_drawio.py), so a change to an agent's
...
![Multi-Agent NL2SQL v5](Multi-Agent_NL2SQL_arch5.png)
...
Completeness Reviewer (its reflection), Insight Narrator.
...
flowchart TB
    ...
    subgraph S1["Stage 1 · Intake and context gathering"]
        direction TB
        SUP["Supervisor · LLM<br/>in-domain? injected? ambiguous? · intent class · answer contract"]
    ...
    subgraph S2["Stage 2 · Synthesis"]
        GEN["SQL Generator · LLM, multi-shot<br/>system: rules + pruned schema · turns: exemplars · human: question + answer contract + repair hint"]
    end
    ...
    subgraph S3["Stage 3 · Validation and repair loop"]
        ...
        REV["Completeness Reviewer · rules, then LLM<br/>label beside every key? ranked measure present? period stated? · what would the reader ask next?"]
...
|---|---|---|
| Chat model `qwen3.8-256k`, 256k context | Ollama at `192.168.10.82:11434` | Supervisor, SQL Generator, Completeness Reviewer (reflection), Insight Narrator, Repair Agent (unclassified errors only) |
...
schema's *column names only*. It returns structured output:
...
| Output | Narrative claims are verified against rows; sensitive columns are aggregated only; every default the pipeline chose for the user is stated. Markdown output is rendered with HTML escaped, which is where the XSS concern from arch3 actually belongs. | Completeness Reviewer, Audit Checker, renderer |
...
`MAX_ATTEMPTS`, `MAX_PLAN_COST`.

---

## 12. Change log against arch1-4
