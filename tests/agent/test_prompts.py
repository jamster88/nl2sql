"""Prompt templates and the knowledge block that v2 threads through them.

These are the seam between retrieval and the model: if a template stops
accepting `{knowledge}`, or the block renders when there is nothing to say,
the failure shows up as a confusing model answer rather than an exception --
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            SQL_GENERATION_PROMPT,
            {"dialect": "postgresql", "schema": "s", "question": "q", "feedback": ""},
        ),
        (
            SQL_VALIDATION_PROMPT,
...
def test_generation_prompt_tells_the_model_to_prefer_knowledge_over_assumptions():
    text = render(
        SQL_GENERATION_PROMPT.format_messages(
            dialect="postgresql", schema="s", knowledge="", question="q", feedback=""
        )
    ).lower()
